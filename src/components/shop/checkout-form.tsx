...
export default function CheckoutForm() {
  ...
  // Initialize form data with empty strings and then update with user data if available
  ...
  // Update form data with user information when user changes
  useEffect(() => {
    if (user) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If user is not logged in, create a new customer or find existing one by email
      if (!isAuthenticated) {
      ...
      // Create order items for the database
